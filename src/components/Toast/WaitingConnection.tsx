...
import { WaitingConnectionContent } from './ToastVariants/WaitingConnectionContent'
...
export class WaitingConnection extends Component<PropsType, StateProps> {
  static _ref: any = null
  timer: NodeJS.Timeout | null

  static setRef(ref: unknown): void {
    WaitingConnection._ref = ref
  }

  static getRef(): void {
    return WaitingConnection._ref
  }

  static clearRef(): void {
    WaitingConnection._ref = null
  }

  static show(options: ToastType): void {
    WaitingConnection._ref?.show(options)
  }

  static hide(): void {
    WaitingConnection._ref?.hide()
  }

  static status = 'hidden'
  ...
  animate({ toValue, type }: { toValue: number; type: 'hide' | 'show' }): Promise<void> {
    WaitingConnection.status = 'inProgress'
    ...
    return new Promise((resolve) => {
      if (type === 'show') {
        ...
        Animated.spring(animation, config).start(() => {
          resolve()
          WaitingConnection.status = type === 'show' ? 'shown' : 'hidden'
        })
      }
      ...
      if (type === 'hide') {
        ...
        Animated.timing(animation, config).start(() => {
          resolve()
          WaitingConnection.status = type === 'hide' ? 'hidden' : 'shown'
        })
      }
    })
  }
      ...
      >
        <View>
          <View style={[styles.parent, animateFrom === 'bottom' && { bottom: 0 }]}>
            <WaitingConnectionContent onLayout={this.onLayout} state={this.state} />
          </View>
        </View>
      </KeyboardAvoidingView>
    )
